...
void CAutoGoState::Update(float dt)
{
	CMario* mario = CMario::GetInstance();
	CGame* game = CGame::GetInstance();
	//mario->x = game->GetCamX();
	mario->vx = 0.09f;
...
void CAutoGoState::HandleKeyboard()
{

}

void CAutoGoState::SetAnimation(int level)
{
	this->level = level;
	CMario* mario = CMario::GetInstance();
	switch (level) {
...
void CAutoGoState::OnKeyDown(int KeyCode)
{
	CPlayerState::OnKeyDown(KeyCode);
}

void CAutoGoState::OnKeyUp(int KeyCode)
{
	CMario* mario = CMario::GetInstance();
}

void CAutoGoState::KeyState(BYTE* state)
{
	CMario* mario = CMario::GetInstance();
	CGame* game = CGame::GetInstance();
